...
-- 정답: 1번(X) 이건 카테시안 곱(전부 곱해버리는) , 2번(O)
...
-- ① 두 테이블 간 모든 튜플을 결합한다.
-- ② 두 테이블을 결합할 때 빈 칼럼은 NULL 값으로 채운다. (OUTER JOIN의 특징)
-- ③ MySQL에서는 지원하지 않는다.
...
-- ① 두 테이블의 모든 칼럼이 동일한 값을 가져야 한다. (꼭 동일한 값을 안가져도됨)
-- ② 결과 집합에 중복 데이터를 포함시키지 않으려면 UNION ALL을 사용해야 한다. ( UNION)
-- ③ 두 테이블의 결과 집합에 포함된 칼럼 수와 각 칼럼의 자료형이 동일해야 한다.
-- ④ UNION 연산자는 테이블 간의 관계를 기반으로 데이터를 결합한다. (UNION은 관계랑은 상관없음, 그저 위아래로 붙임)
...
-- 정답: 
SELECT name, date_of_birth, address -- 별칭해주는게 좋은습관
FROM members m
...
-- 정답:
SELECT b.title, br.borrow_date -- b.랑 br.은 명시적으로 가독성을 위해 적음
FROM books b
...
SELECT m.name
FROM members m
LEFT JOIN library_staff ls ON m.id = ls.id
)
UNION 
(
SELECT ls.name
FROM members m
RIGHT JOIN library_staff ls ON m.id = ls.id
);
