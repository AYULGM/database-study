...
-- ① 특정 칼럼의 값이 항상 고유해야 함을 보장한다. (유니크)
-- ② 특정 컬럼의 값이 NULL을 허용하지 않도록 제한한다.(낫 널)
-- ③ 새 행이 삽입될 때마다 값을 1씩 자동으로 증가한다.
-- ④ 칼럼 값이 음수를 허용하지 않도록 제한한다.(언사인드)
...
CREATE TABLE books (
    ...
    -- stock INTEGER UNSIGNED DEFAULT '0' -- 이렇게 쓰면 MySQL은 정수 0으로 자동변환시켜주는데 다른 DBMS에는 오류가 날 수 있음.
	PRIMARY KEY (id) -- 기본키 지정: id
);
...
CREATE TABLE borrow_records (
	...
	PRIMARY KEY (id) -- 기본키 지정: id
	FOREIGN KEY (member_id) REFERENCES members(id) -- 외래키 지정: member_id
	FOREIGN KEY (book_id) REFERENCES members(id) -- 외래키 지정: book_id
);
...
CREATE TABLE library_staff (
	...
	salary INTEGER CHECK (salary >= 0), -- 급여(음수 불가, CHECK 제약 조건 사용) , UNSIGNED는 자료형자체에서 막아버리는거고 CHECK(salary>=0)는 검사할때 체크해서 막는거임.
	PRIMARY KEY (id) -- 기본키 지정: id
);
...
-- 정답: 3번 (NOT NULL은 안줘서 NULL은 들어갈수있음)
...
-- 정답: 4번 ( UNIQUE안주면 1:N (일대다) 관계가 되버림)
...
-- 정답: 3번 ( 중간테이블이 되어서 다대다 관계가 성립하게끔)
