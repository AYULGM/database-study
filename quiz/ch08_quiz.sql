...
-- ④ 데이터를 필터링하여 출력할 때 사용한다.(where=개별행단위로 , having=그룹단위로)
...
-- 정답: 3번 ( 모든 DB에서 동작하는 방식 - SELECT 이후 ORDER BY가 실행되기때문에 SELECT에서 쓴 별칭을 쓸수있음)
...
FROM books
GROUP BY category
HAVING total_stock <= 10; -- 순서는 HAVING다음 SELECT인데 MYSQL에서는 허용해줌, 대부분의 DB에서 허용 해주긴하는데 좀 더 안전한 방식은 HAVING절에선 별칭을 안쓰는것.
...
SELECT YEAR(join_date) AS join_year, COUNT(*) AS new_members -- 3)
FROM members -- 1)
GROUP BY join_year -- 2) GROUP BY도 이렇게 별칭을 가져다쓰는건 MySQL계열만 되는걸로 아신다고 함
ORDER BY new_members DESC; -- 4) 이건 SELECT뒤라 별칭 가져다쓰는거 모든 DB에서 허용
