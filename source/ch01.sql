...
*/
-- MySQL 서버 & MySQL 워크벤치
-- (서버)       (클라이언트) , 클라이언트는 뭐 자유롭게 고르는 사람있다고함. 서드파티 쓰는사람도있고 VSCODE에서 확장프로그램으로 클라이언트 이용하는사람도 있고.
...
-- 기능적으로는 소문자든 대문자든 차이가 없고(SQL은 대소문자 구분안함), 전적으로 가독성과 팀의 코딩 스타일 가이드에 따라 달라짐
...
-- NoSQL(Not Only SQL) 데이터베이스 관리 시스템 .. MongoDB가 대표적..JSON을 쓴다.
-- - NoSQL은 "Not Only SQL"의 줄임말
-- - 빅데이터와 특정 요구사항에 맞춰 최근 등장한 시스템
-- - 예: 엄청나게 많은 비정형(구조화 안된) 데이터 처리, 매우 빠른 읽기/쓰기 속도, 유연한 데이터 모델(장점이자 단점) 등
-- - 관계형 데이터베이스의 한계를 보완