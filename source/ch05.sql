...
-- 종류: TINYINT(1byte), SMALLINT(2byte), MEDIUMINT(3byte), INTEGER(또는 INT)*(4byte), BIGINT(8byte)

-- 정수에는 필요하면 UNSIGNED를 붙일수있다.
-- UNSIGNED(언사인드) 제약 조건 부여 가능: 음수 값을 허용하지 않는 정수
-- 유효성이나 안전성이나 비슷비슷하긴하다
...
CREATE TABLE student_records (
    ...
    average_score FLOAT, -- 평균 정수(부동 소수점 방식의 실수)
...
-- grade 컬럼에 자료형 범위를 벗어난 값을 입력한다면 => 에러 발생!
INSERT INTO 
	student_records (id, grade, average_score, tuition_fee)
VALUES
	(3, -2, 66.5, 20.00); -- -2(음수값)를 집어넣으려해서 에러
    
INSERT INTO 
    student_records (id, grade, average_score, tuition_fee)
VALUES
	(4, 256, 66.5, 20.00); -- 255를 초과한 256를 집어넣으려해서 에러
...
CREATE TABLE addresses (
	postal_code CHAR(5), -- 우편번호(고정 길이 문자: 5자) , 우리나라는 우편번호 5자로 고정이니까.
...
-- 2) TEXT
-- 긴 문자열 저장을 위한 타입
-- CHAR/VARCHAR로는 감당하기 어려운 매우 긴 텍스트 데이터를 저장하기 위해 존재 , 근데 웬만하면 VARCHAR로 충분하다고는 하심
...
-- TEXT 자료형의 사용 예
CREATE TABLE articles ( -- 꼭이렇게 쓸 필욘없다. 그저 예시일뿐임. 자주쓰는건 TEXT래.
...
CREATE TABLE user_profiles (
    ...
    profile_picture BLOB, -- 프로필 사진(파일: 최대 16MB) -- BLOB이아니라 MIDIUMBLOB으로 해야됨
...
-- LIKE '케첩_': '케첩'으로 시작하는 3글자 => 케첩병, 케첩통
...
-- 예: 서울특별시에 살지 않는 고객들만 보여달라.
SELECT *
FROM customers -- 고객테이블이 있다고 가정을하고
WHERE address NOT LIKE '서울특별시%'; -- 주소가 서울특별시로 시작하지않는 고객들

-- 2) 문자열 함수    (필터링 기능은 아닌데 지금 배우면 좋을거같아서 넣었다고 하심)
-- CONCAT(문자열1, 문자열2, ...): 문자열 합치기, 괄호 안에 전달된 여러 문자열을 순서대로 이어 붙여 하나의 문자열로 만듦
-- CONCAT은 concatenate의 약자 : 사슬같이 잇다, 연쇄시키다  라는뜻
...
SELECT *
FROM orders
WHERE HOUR(created_at) < 12; -- 11:59:59 까지 오전이니까.
...
SELECT *
FROM orders
WHERE price BETWEEN 10000 AND 20000; -- 만원 '이상' 2만원 '이하' , BETWEEN은 포함범위임(>= , <=)
...
-- 가(AC00) ~ 깋(AE4B) 범위에 해당
SELECT *
FROM orders
-- WHERE name >= '가' AND name <= '깋'; -- 이렇게하면 깋내산 고추장이 검색이 안됨
-- WHERE name BETWEEN  '가' AND  '나'; -- 이렇게 하면 '나'로 시작하는 '나무' 이런것도 검색이 됨
WHERE name >= '가' AND name < '나'; -- 'ㄱ자형 책상'도 검색이 안되는데 그러면 '가'대신 'ㄱ'으로 바꾸면되긴하는데 'ㄱ'으로 시작하는건 진짜 드무니까 그냥 '가'로 시작함

-- 주의! 위 조건과 다른 의미
SELECT *
FROM orders
WHERE name LIKE 'ㄱ%'; -- 말 그대로 'ㄱ'으로 시작하는 문자열 검색 (ㄱ자형책상 만 검색이됨)
...
SELECT * FROM orders; -- 데이터 잘 들어간거 확인
