...
--     PRIMARY KEY (컬럼명)
-- );
CREATE TABLE burgers ( -- 테이블명 뒤에 s(복수형) 붙이는건 관례
	id INTEGER, -- 아이디(정수), INTEGER말고 INT라고 써도됨
    -- id INT PRIMARY KEY, -- 또는 이렇게 써도 가능 (컬럼지정 + 기본키를 id로 지정) 
    name VARCHAR(50), -- 이름(최대 50글자의 문자)
    -- VARCHAR(N): 문자를 최대 N자리까지 저장(가변 길이 타입이라 저장될 수 있는 최대 길이의 제한을 알려줘야 함),나중에 배울거임
    price INTEGER, -- 가격(정수)
    gram INTEGER, -- 무게(정수)
    kcal INTEGER, -- 열량(정수)
    protein INTEGER, -- 단백질량(정수)
    PRIMARY KEY (id) -- 기본키 지정(필수!): 기본키로 id라는 컬럼을 지정함(기본키는 고유해야 하니까)
);
-- 기본키: 레코드(row)를 대표하는 컬럼(예: 사람의 주민등록번호, 학생의 학번, 군인의 군번, 사원의 사번) - 고유하게 구분할 수 있는 대표적인 값들
-- 테이블에 저장된 모든 버거를 구분하기 위한 컬럼
-- 중복되지 않는 값을 가짐

-- 테이블 구조 조회
-- DESC 테이블명;  -- DESC는 DESCRIBE(=묘사하다,서술하다,말하다)
...
-- burgers 테이블에 set-price(세트 메뉴 가격) 컬럼 추가
ALTER TABLE burgers
ADD COLUMN set_price INT NOT NULL DEFAULT 0; -- 데이터형은 INT형이고 NULL이 들어가면 안되고 기본값을 0으로 지정하겠다.
...
DESCRIBE burgers; -- DESC라고 줄여서 해도됨
...
VALUES (1, '빅맨', 5300, 223, 589, 27); -- 문자열을 넣을땐 작은 따옴표

-- 데이터 조회하기
-- SELECT 컬럼명1, 컬렴명2, ... -- 조회할 컬럼(컬럼명 대신 *을 쓰면 모든 컬럼 조회)
-- FROM 테이블명
...
-- 컬럼 생략 가능한가? 가능하지만 테이블 정의에 맞게 모든 열에 순서대로 값을 넣어야 함
-- 실무에서는 항상 컬럼을 명시하는 방식을 권장(가독성, 유지보수, 디버깅을 위해 컬럼을 명시하자)
INSERT INTO burgers
VALUES (2, '빅맨', 5300, 942, 194, 85);

-- 데이터를 넣지 않으면 NULL이 들어감
INSERT INTO burgers (id, name, gram, kcal)
VALUES (3, '빅맨', 182, 444);

INSERT INTO burgers (id, name, gram, protein)
VALUES (4, '빅맨', 182, 444);

-- 테이블의 모든 데이터 삭제 (테이블 비우기) , DELETE를 통해서도 지울 수 있는데 TRUNCATE쓰는게 훨씬 빠름 , 왜 인지는 나중에 배움
TRUNCATE TABLE burgers;

-- 다중 데이터 삽입하기 , 문장이 길면 들여쓰기 해
INSERT INTO 
...
-- 모든 값을 일괄적으로 변경하는 것을 클라이언트에서 막고 있음 (혹시라도 모든 컬럼을 삭제/변경할까봐, 모든 row를 삭제/변경할까봐 클라이언트에서 안전모드로 보호해주고 있음)
-- 에러 메시지 확인: 안전 모드에서는 KEY 없는 UPDATE/DELETE를 차단함

-- 안전모드 설정확인
SELECT @@sql_safe_updates; -- 0: 해제, 1:설정  , 현재 설정되어있음(1)

-- 임시로 안전모드 해제(권장 안함, 우리는 학습용도니까 잠깐씀)
