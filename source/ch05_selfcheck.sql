...
SELECT COUNT(*)
FROM orders
-- WHERE MONTH(created_at) = 11 AND created_at > '2024-11-20'; -- 난 이렇게하고
WHERE MONTH(created_at) = 11 AND DAY(created_at) > 20 ; -- 쌤은 이렇게하심

-- 4. 상품명에 부피 단위인 'ml' 또는 'l'가 포함된 주문을 모두 조회하세요.
SELECT *
FROM orders
WHERE name LIKE '%ml%' OR name LIKE '%l%'; -- '%l%'가 '%ml%' 결과를 포함하기에 중복된 조건이다.

SELECT *
FROM orders
WHERE name LIKE '%l%'; -- 그래서 이렇게 중복조건을 제거함
...
-- IN 연산자 사용 버전 ( OR은 IN을 쓰면 더 간단해진다고 설명했음 )
SELECT COUNT(*)
FROM orders
