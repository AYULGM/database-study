...
FROM students
WHERE score > (
	-- 서브쿼리: 평균 점수 계산 (한번 실행된 서브쿼리 결과값을 메모리에 저장해놓고 비교함), WHERE절은 행마다 실행되는게 아님
    SELECT AVG(score) AS '학생들의평균점수'
	FROM students
);

-- 서브쿼리의 특징 5가지 - 암기X , 이해O
-- 1) 중첩 구조
-- 메인쿼리 내부에 중첩하여 작성
SELECT 컬럼명1, 컬럼명2, ...
FROM 테이블명
...
-- 4) 단일 값 또는 다중 값을 반환
-- 단일 값 서브쿼리: 특정 값을 반환하는 서브쿼리(1행 1열), SELECT 문에서 쓰임
-- 다중 값 서브쿼리: 여러 레코드를 반환하는 서브쿼리(N행 M열) , N >= 1 , M >= 1
-- 가상의 테이블로 쓰이거나 IN, ANY, ALL, EXIST 연산자와 함께 사용됨, FROM/JOIN, WHERE/HAVING 등 에서 쓰임
...
SELECT
	payment_type AS '결제 유형',
    amount AS '결제 금액',
    amount - (SELECT AVG(amount) FROM payments) AS '평균 결제 금액과의 차이' -- 결국 SELECT AVG(amount) FROM payments의 값이 17910이므로 17910을 넣은것과 똑같음.
FROM payments;
...
SELECT
	payment_type AS '결제 유형',
    amount AS '결제 금액',
    amount - (SELECT AVG(amount), '123' FROM payments) AS '평균 결제 금액과의 차이' -- 1행 2열(에러뜸)
    -- Error Code: 1241. Operand should contain 1 column(s) -- 1열만 들어가야한다.
    amount - (SELECT amount FROM payments) AS '평균 결제 금액과의 차이' -- 7행 1열(에러뜸)
    -- Error Code: 1242. Subquery returns more than 1 row	-- 1행만 들어가야한다.
FROM payments;


-- 2. FROM 절에서의 서브쿼리
-- NxM(N행M열)에 반환하는 행과 컬럼의 개수에 제한이 없음
-- 서브쿼리를 이용해 가상의 테이블(View)을 만들어 사용하기에 테이블 서브쿼리(or 인라인 뷰)라고 부름
-- 단, 서브쿼리에 별칭 지정 필수

-- 1회 주문 시 평균 상품 개수는? (장바구니 상품 포함)
-- 일단 먼저 1회 주문 당 상품 개수 집계 구하기
-- 주문별(order_id)로 그룹화 -> count 집계: SUM() -> 재집계: AVG()   두번 나눠서 할수도있는데 이걸 한 쿼리안에 하는게 서브쿼리

-- 일단 주문별(order_id)로 그룹화 -> count 집계: SUM() 까지 서브쿼리 하나 만들어보자!
SELECT 
	order_id,
...
-- (sub. 는 명시)sub.total_count대신 SUM(count)라고 적으면 집계함수안에는 값이 들어가야하는데 집계함수 안에 집계함수 AVG(SUM(COUNT)) 이딴식으로 들어가면 에러
-- 그래서 별칭으로 넣어주는거임
FROM (
	-- 서브쿼리
	SELECT 
...
SELECT
	p.name AS 상품명, -- SQL표준원칙상 name으로 못쓰고 p.id라 써야하는게 원칙이긴함
    SUM(count) AS '주문 개수'
FROM order_details od
JOIN products p ON od.product_id = p.id
GROUP BY p.id, p.name; -- p.id와 p.name이 1:1로 매칭이기때문에 id로 그룹화하면 name을 적은것과 똑같음
-- 원칙상으로는 p.id와 p.name 둘다 써줘야함



-- 4. WHERE 절에서의 서브쿼리
-- 1x1, Nx1 반환하는 서브쿼리만 사용 가능 -- 생각해보면 당연한거
...
SELECT 
	p.name AS 상품명, 
	SUM(p.price * od.count) AS '상품별 매출'
FROM products p
...
-- 정답: O O(FROM,JOIN은 테이블명이 와야함, 그래서 별칭 지정은 필수) O
...
FROM order_details
WHERE product_id IN (
-- 서브쿼리: 우유 식빵과 크림 치즈의 상품 아이디를 반환(Nx1) <-- 다중행x컬럼하나 
    SELECT id
	FROM products
...
-- ANY는 하나라도 만족, ALL은 모두 만족해야함
-- ANY,ALL보단 MIN,MAX가 더간단함
...
-- 결과행이 1개라도 발견되면 바로 TRUE (서브쿼리 중단)
...
SELECT *
FROM users u
WHERE EXISTS ( -- 행단위로 필터링
	-- 서브쿼리: 주문자 아이디가 사용자 테이블에 있으면 숫자 1 반환
    SELECT 1 -- 굳이 모든 컬럼(*)을 다 가져오는 것보다 1만 써주는게 효율적이고 관례 (나 존재한다 라는의미로 단순하게 숫자 1만 쏴주는거임, 속도 차이나니까.)
    FROM orders o
    WHERE o.user_id = u.id -- 이렇게 서브쿼리가 메인쿼리의 특정 값을 참조하는 쿼리를 '상관 서브쿼리'라고 함, (u.id는 고정된값이 아니고 계속변함.)
);               
-- (참고)
-- users 테이블 1번 사용자의 주문이 있는지
-- orders 테이블을 반복하며 확인(마치 이중 for문 돌리듯), 있으면 결과 테이블에 1 반환
-- EXISTS는 결과가 하나라도 존재하면 TRUE가 되기 때문에, 매칭되는 레코드를 찾으면 더 이상 검사하지 않음(찾았으면 멈추고 TRUE를 반환)
-- 같은 방식으로 2번, 3번 사용자도 확인

-- (정리) 상관 서브쿼리의 동작 원리
-- 비상관 서브쿼리: 서브쿼리가 단 한 번 실행된 후(메모리에 값을 가지고있음), 그 결과를 메인쿼리가 사용
-- 상관 서브쿼리:
-- 1) 메인쿼리가 먼저 한 행을 읽고
...
FROM orders; -- 적어도 한번이상 주문한 사용자 (1,2,3)

SELECT *
FROM users
WHERE id IN (
	SELECT DISTINCT user_id
	FROM orders -- 서브 쿼리의 결과값이 많아질수록 성능 저하
); -- 하드코딩하면 (1,2,3) 을 서브쿼리로 표현....
