...
SELECT *, MAX(price), MIN(price)
FROM burgers; -- 오류 나옴
-- 이 둘은 동시에 쓸 수 없음, 왜냐하면 집계는 그룹 단위 연산이고 *는 행 단위 열람이기 때문, *뿐만 아니라 일반 컬럼들도 못씀
-- "어떤 행의 값을 보여줄지?" MySQL이 결정할 수 없어서 에러를 뱉음
-- Error Code: 1140. In aggregated query without GROUP BY, expression #1 of SELECT list contains nonaggregated column 'mapdonalds.burgers.id'; this is incompatible with sql_mode=only_full_group_by
-- 집계 함수를 다른 일반 컬럼들과 함께 사용할 때는 GROUP BY가 필요

-- 해결 방법
-- 1: 집계되지 않은 일반 컬럼(*, name ,price 같은거)은 제거

-- 2: 그룹별로 집계(예: 카테고리별 - 소고기, 치킨 등) , 나중에 배움
SELECT category, MAX(price), MIN(price)
FROM burgers
GROUP BY category;
-- category 별로 그룹을 묶고, 각 그룹에서 가격의 최댓값/최솟값을 구함

-- 3: 서브쿼리(쿼리안에 쿼리작성)를 활용하여 최대/최소 가격을 가진 버거 정보까지 보고 싶을 때, 나중에 배움
SELECT *
FROM burgers
-- WHERE price = 7700 OR price = 5300; -- 이렇게 작성하면 항상 가장 비싼버거,싼버거의 가격을 하드코딩해서 작성해야함
WHERE price = (SELECT MAX(price) FROM burgers) OR price = (SELECT MIN(price) FROM burgers);
...
-- 모든 직원 수 세기 (id는 기본키라 null이 들어갈 수 없기 때문에 전체 레코드를 셀 수 밖에 없다.)
SELECT COUNT(*)
FROM employees;
...
SELECT AVG(price)
FROM burgers;
-- (참고) 계산 과정에서 NULL은 자동으로 제외함 (NULL을 0으로 취급하지 않음, 그러기 때문에 나눌때 분모에 +1을 안하고 건너뜀)
...
CREATE TABLE transactions (
	id INTEGER, 			-- 아이디
	amount DECIMAL(12, 2), 	-- 거래 금액(12자릿수: 정수 10자리까지, 소수점 이하는 2자리까지 허용) -- float,double은 부동소수점(가수부,지수부)인데 DECIMAL은 고정소수점(소수점이 고정)이다.
	msg VARCHAR(15), 		-- 거래처
...
VALUES
	(1, -24.20, 'Google', '2024-11-01 10:02:48'), -- DATETIME을 입력할 때 포맷(형식)은 지정된거임, 바꿀 수 없음
	...
	(27, -480.78, 'Bank of America', '2025-02-14 17:30:12'); -- -는 돈이 출금, +는 돈이 예금된거임
...
-- 위 쿼리를 IN 연산자(목록에 포함된 값 찾기)를 활용한 버전으로 다시 작성한다면?     IN 많이 쓰니까 잘 알아두셈
SELECT COUNT(*)
FROM transactions
WHERE msg IN ('Coupang', 'Amazon');
-- IN 의미: msg가 () 안에 포함이 되어 있다면 TRUE
-- IN 연산자를 사용하면 훨씬 더 직관적이고 편리함 (OR가 계속 붙게되면 코드가 복잡해짐)
...
SELECT AVG(amount)
FROM transactions
-- WHERE (msg = 'Google' AND amount > 0) OR (msg = 'Amazon' AND amount > 0);
-- WHERE (msg = 'Google' OR msg = 'Amazon') AND amount > 0;
WHERE msg IN ('Google', 'Amazon') AND amount > 0; -- 점점 더 깔끔해지는 코드

-- 거래처 목록 조회하기
SELECT msg
FROM transactions;
-- 거래처를 담은 msg만 조회하면? 중복된 결과가 나옴 -> 27개 데이터가 전부 나온다.
...
SELECT DISTINCT COUNT(msg) -- msg의 개수 27에 대한 중복 제거 (아무 의미없는 쿼리, 사람들이 자주하는 실수)
FROM transactions;
