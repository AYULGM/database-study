...
-- customers, orders, products  테이블에서 주문 내역이 필요하기 때문에 orders의 정보가 필요 ( orders가 두 테이블 사이에서 다대다의 중간테이블 역할 )
...
-- 기본키(PK)와 외래키(FK)를 써서 테이블을 연결하는 것이 가장 "일반적"이고 기본이 되는 방식
-- 그러나 조인의 핵심 원리는 '두 테이블의 특정 열(column)의 값이 같으면' 연결 (굳이 PK와 FK없어도, 관계를 맺지않아도 자료형만 같으면된다. 둘다 '문자'거나 , '숫자'거나....)
...
JOIN 테이블B -- 연결할 두번째 테이블 지정(JOIN만 명시하면 기본적으로 INNER JOIN으로 해석, 뒤에서 배움)
...
-- Quiz: 1번 사진에 달린 모든 댓글 본문과 작성자 닉네임 가져오기 (join 이후 where로 필터링)
SELECT nickname, body -- 4) 닉네임과 본문을 조회한다.
FROM comments -- 1) 댓글 테이블의
...
-- 조인의 특징 7가지
-- 1) 조인 컬럼이 필요
-- 두 테이블을 연결하기 위한 공통 컬럼이 필요 ( 말이 안되지만 상품테이블의 price인 500, 2000과 users의 id인 500, 2000을 이어 붙일 수 있다.(같은 INT 데이터 형) 전혀 영양가 없는 데이터가 조인이 됨)
-- 보통 외래키와 기본키를 기준으로 조인 (관련있는 데이터)
SELECT *
FROM comments
...
-- 다만 DBMS마다 자동 형 변환이 될 수도 있음(문자열 '1'을 숫자 1로 바꿔줌. 원칙상 안되는게 맞음)
...
-- 사용 예: 중복 컬럼id에 테이블명 명시
SELECT 
	comments.id AS comment_id, -- 이렇게 줄바꿈, 별명 지어주면 더 깔끔해지고 가독성 올라감
    body,
    users.id AS user_id,
    nickname
FROM comments
...
-- 실무에서는 어떤 테이블의 컬럼인지 명시하는 것이 가독성을 높이고, 쉽게 인지할 수 있기 때문에 사용하는 것을 권장 ( 알고있는게 좋음)
-- 다음에 나오는 테이블 별칭과 함께 사용하는 것이 좋음

-- 6) 테이블명에 별칭 사용 가능 (권장 방식)
-- 간결한 쿼리 작성 및 가독성 향상에 도움
SELECT 
    c.body,
    u.nickname
FROM comments AS c -- 테이블의 앞자리 1글자를 따는게 관례
JOIN users AS u ON c.user_id = u.id;
-- 실무에서 테이블에 별칭을 붙일 때는 AS를 보통 생략 ( 안써주는게 일반적)
-- (참고) SELECT 절에서 컬럼에 붙이는 별칭은 AS를 생략 안함 (가독성을 위해 SELECT에선 AS 생략X)

-- 7) 다양한 조인 유형 사용 가능
-- 조인은 크게 내부 조인(INNER JOIN)과 외부 조인(OUTER JOIN)으로 나뉨
-- 다양한 결과 테이블 생성에 도움
-- INNER 조인(풀네임: INNER JOIN) , 조인의 기본형은 INNER 조인 (그냥 JOIN이 INNER JOIN임)
-- LEFT 조인(풀네임:  LEFT OUTER JOIN)
-- RIGHT 조인(풀네임:  RIGHT OUTER JOIN)
-- FULL 조인(풀네임:  FULL OUTER JOIN)
...
-- 정답: 1번(자료형이 다른데도 만약 된다면 DBMS가 자동 형 변환 해준거임)
...
SELECT 컬럼명1, 컬럼명2, ...
FROM 테이블A
INNER JOIN 테이블B ON 테이블A.조인_컬럼 = 테이블B.조인_컬럼; -- 실제로 쓸때는 INNER 생략

-- Quiz: photos 테이블과 users 테이블 INNER 조인 -- 교집합이라 테이블A에 B를 더하든 B에 A를 더하든 똑같음 (순서만 바뀜)
SELECT *
FROM users
...
-- 2. 외부 조인(OUTER JOIN)
-- 두 테이블 간의 조인 결과에 누락된 행을 포함시킬 수 있는 조인 방식 (INNER JOIN에는 없던 NULL도 표시한다.)
...
-- 조인 조건을 만족하지 않는 경우, NULL 값으로 채움
-- LEFT 조인에서 기준만 바뀌고 동일하기에 거의 사용되지 않음 (LEFT조인에서 그냥 테이블A, B위치만 바꾸면 되서 굳이 RIGHT조인 필요없음)
...
-- 실무에서는 잘 사용하지 않고, MySQL은 FULL 조인을 지원하지 않음 ---------> 그래서 MySQL에서는 UNION으로 구현
-- 형식:
SELECT 컬럼명1, 컬럼명2, ...
FROM 테이블A
...
-- SELECT 결과를 단순히 '위아래'로 붙이는 연산 ( JOIN은 옆으로 붙임)
-- UNION을 사용하려면 반드시 지켜야 할 규칙
-- 1) UNION으로 연결되는 모든 SELECT 문은 "컬럼 갯수가 동일" 해야 하고,
...
-- (참고) UNION 연산을 활용하면 FULL 조인 결과를 생성 가능 (UNION 연산자는 중복된 데이터 있으면 다 제거해준다.)
-- photos 테이블과 users 테이블 FULL 조인 = LEFT 조인 + RIGHT 조인 + 중복 제거
(
	SELECT *
	FROM photos p	
	LEFT JOIN users u ON p.user_id = u.id
...
-- 잘 안쓰이지만 CROSS JOIN( M(행)*N(행) ) , SELF JOIN(같은 테이블을 별칭을 다르게)도 말하심
...
-- (참고) 또는 필터링을 조인과 동시에 가능 ----> 쌤은 가독성을 위해 위에 있는 WHERE이 있는 문장을 선호한다고 하심
SELECT 
	filename AS 게시자, 
    nickname AS 파일명
FROM photos p
...
SELECT body '댓글내용', filename AS '파일명' -- 웬만하면 SELECT에는 가독성을 위해 AS를 붙이자.
FROM comments c
...
-- 정답: O X(JOIN조건을 만족하는 애들만) O O (WHERE로 해도되고 JOIN과 동시에 AND해도되고..)
