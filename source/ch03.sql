...
INSERT INTO burgers (id, name, price, gram, kcal, protein)
VALUES
	(6, null, 5900, 250, 605, 33); -- 이런 경우는 없겠지만 햄버거 이름을 몰라서 null을 집어넣었다고 가정하자.
SELECT * FROM mapdonalds.burgers;
...
SELECT *
FROM burgers
WHERE name IS NULL; -- 잘 나온다.

SELECT *
FROM burgers
WHERE name IS NOT NULL; -- NULL이 아닌애들도 잘 나온다.
...
SELECT *
FROM burgers
WHERE price < 5500 AND protein > 25; -- AND가 표준이고 &&(기호)로 쓰면 안되는 SQL도 있음

-- Quiz: 5500원 보다 싸거나, 단백질량이 25g 보다 많은 버거
SELECT *
FROM burgers
WHERE price < 5500 OR protein > 25; -- 이것도 마찬가지로 OR가 표준임

-- QUiz: 단백질량이 25g 보다 많지 않은 버거
SELECT *
FROM burgers
WHERE NOT protein > 25; -- 이것도 마찬가지로 !(기호)로 쓰는것보단 NOT이란 표준을 쓰는게 나음
-- WHERE !(protein > 25); -- (참고) &&, ||, !는 MySQL에서만 작동
-- 가독성과 이식성을 위해서는 AND, OR, NOT을 사용하는 것이 가장 안전
-- PostgreSQL, Oracle, SQL Server 등 다른 DBMS에서는 에러 남
...
SELECT *, price / gram * 100 AS `price/100g` -- 별칭에 특수문자나 공백이 들어가면 따옴표("", '') 또는 백틱(``)으로 감싸기
FROM burgers;
...
-- 괄호 > 산술(*, /, %) > 산술(+, -) > 비교 > 논리(NOT) > 논리(AND) > 논리 (OR)

-- Quiz: 다음 쿼리의 수행 결과는?
SELECT 3 + 5 * 2; -- 13 (곱하기 우선)
SELECT (3 + 5) * 2; -- 16 (괄호 우선)
SELECT TRUE OR TRUE AND FALSE; -- 1 (AND가 우선)
SELECT (TRUE OR TRUE) AND FALSE; -- 0 ( 괄호가 우선)
...
SELECT 
	nickname AS 닉네임, 
    math+english+programming AS total, 
    (math+english+programming)/3 AS avarage
FROM students
...
-- 정답: 2번(English>90이 참이면 math가 빵점이어도 조회되서 2번은 틀림),5번 ( 괄호 제거하면 programming/3 이 먼저 계산되고 math와 english가 더해져서 5번은 틀림)
