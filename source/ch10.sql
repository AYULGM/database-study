...
-- 데이터베이스를 만들기 위해서 기본 설계도를 그리는 것 ( 적어도 DB 명세서 만들기 )
-- 목표는? 데이터의 일관성, 중복 최소화, 효율적인 관리 (그래서 정규화를 한다.)
...
-- 2) 의사소통에 기여
-- - 전문가(개발자)와 비전문가(기획자, 디자이너 등) 사이에서 소통의 도구
...
-- 1) 개념적 데이터 모델링
-- 현실 세계를 구성하는 주요 개체(엔티티(테이블들), entity)와 그들 간 관계(relation)를 정의
...
-- 예: 
-- 엔티티의 속성(나중에 테이블의 컬럼) 및 식별자(PK) 정의
...
-- 인스턴스: 엔티티의 실제 데이터, 테이블에 저장된 행 단위 데이터 (row, 레코드,튜플)을 설계단계에서의 용어로는 인스턴스
...
-- • 존재성(existence): 값이 반드시 존재해야 함 (이건 NOT NULL)
...
-- (ch10_erd2 이미지 참고)
-- ER 다이어그램 해석1
-- 사용자는 프로필을 가지지 않을 수도 있고, 가진다면 최대 1개 가질 수 있다. (0~1, 선택적 일대일 관계)
-- 프로필은 사용자와 관계를 맺는데, 반드시 가져야 한다.(1~1 , 필수적 일대일 관계)

-- ER 다이어그램 해석2
-- 사용자는 주문을 안 할 수도 있지만, 주문을 한다면 여러 개의 주문을 할 수 있다.(0~N, 선택적 일대다 관계)
-- 주문은 특정 사용자를 참조할 수도 있고, 또는 참조하지 않을 수도 있다.즉, 비회원인 경우에도 주문 가능(0~1, 선택적 일대일 관계)
...
-- ⑤ 등록 인스턴스는 반드시 학생 인스턴스와 강의 인스턴스를 각각 1개씩 참조해야 한다. (그래서 학생ID,강의ID엔 NOT NULL 존재)

-- 정답:  3 (0~N, 선택적 다대다 관계) 학생은없을수도있고 강의가 없을수도있음 그래서 폐강인 강의도있음
...
-- [사용자] --- 주문하다 ---> [주문] --- 결제하다 ---> [결제]
-- [주문] <--- 포함하다(담다) ---> [상품]    주문을통해 상품을 담을수있고 상품은 주문에 담김 당한다
...
-- 각 테이블의 기본키는 id 또는 테이블명_id(테이블명엔 복수형 s를 붙이는게 관례)로, 외래키는 XXX_id로 명명해 일관성을 높임
...
-- user 테이블의 email과 products 테이블의 name을 고유키로 지정
-- 고유키는 UK로 표시(유니크 키)
...
-- 인덱스는 검색 및 정렬 성능을 향상시키는 장점도 있지만, 저장 공간을 많이 차지하는 단점도 있음(그래서 자주 검색이 이용되는 컬럼들만 골라서 인덱스 지정)
...
-- ⑤ __________: 데이터베이스에서 데이터의 검색 및 정렬 성능을 빠르게 해 주는 데이터 구조로 테이블의 특정 컬럼에 생성해 활용
...
-- (ㄴ) 이행적 함수 종속
-- (ㄷ) 완전 함수 종속
-- (ㄹ) 부분 함수 종속 (기본키가 복합키(기본키가 2개이상)일 때 발생 할 수 있음)
-- (ㅁ) 정규화
