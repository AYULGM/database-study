...
-- 예: 사용자-개인 설정, 나라-수도, 사람-주민등록증(id,주민번호,발급날짜,발급한곳,전입주소) <- 주민등록증 자체의 컬럼 자체도 많아서 따로 나누는게 보기편함
...
-- 여러 데이터가 여러 데이터와 연결된 관계('연결 고리' 필요) <<--- 다대다 관계는 중간 테이블이 필요한것임
-- 예: 
-- 1) 학생-과목
-- 한 학생이 여러 과목 수강 가능
-- 한 과목은 여러 학생을 수용 가능
...
-- 다대다(N:M)는 사실상 직접 표현할 수 없기 때문에, "두 개의 일대다 관계"로 분해해서 처리 (일대다 + 일대다 = 다대다)
-- 학생(1)-(N)수강등록(M)-(1)과목
-- 사용자(1)-(N)좋아요(M)-(1)사진
...
-- 테이블 간 관계는 어떻게 만들어질까?
-- 관계를 만드는 키가 존재
-- => 기본키와 외래키의 연결을 통해 만들어짐 ( 마치 제약조건)

-- 기본키(PK, Primary Key)
-- 테이블의 각 행을 고유하게 식별할 수 있는 컬럼
-- 레코드를 구분하는 "컬럼" 또는 "컬럼의 조합"을 말함 ( 두 컬럼 이상을 묶어서 기본키라고 지정할 수도 있다는 것임, 웬만하면 기본키는 1개의 컬럼 or 2~3개 정도만 지정하자.)
-- 예: 주민등록번호
-- 모든 테이블에는 반드시 PK가 존재
...
-- 1부터 시작하여 데이터가 추가될 때마다 1씩 자동으로 증가하는 정수값(INTEGER)을 기본키로 사용하는 경우가 가장 흔함 (auto increment)


-- 외래키(FK, Foreign Key)
-- 다른 테이블의 기본키를 가리키는 컬럼
-- 두 테이블을 관계로 묶어 연결하는 역할
-- 예: 고객 테이블(PK: customer_id, 참조 당하는 쪽, 부모 테이블) <--- (참조) --- 주문 테이블(FK: customer_id, 참조 하는쪽, 자식 테이블)

-- "참조 무결성"을 강제함(두 테이블의 관계가 항상 유효하고 일관된 상태를 유지해야 한다는 원칙)
-- 쉽게 말하면 참조하는 값이 무조건 있어야 함
-- 예: 고객 테이블에 customer_id(PK)가 1~3까지 저장되어 있는데 주문 테이블에 customer_id(FK) 값이 99가 입력된다면(고객 테이블에는 99번 고객 아이디가 없음) 
-- 데이터베이스는 데이터의 입력을 막음(오류 발생) ( 장점으로는 뭐, 단종된 상품같은것을 주문 하려할때 막을 수 있다.)
...
-- 제약 조건의 사용 예 (제약 조건은 보통 애플리케이션 레벨에서 거는게 일반적인데, DB에 제약조건 거는건 최후의 방어선 느낌으로 거는거임)
CREATE TABLE users (
    ...
    PRIMARY KEY(id) 						-- 기본키 지정: id (필수)
);
-- 쌤이 말한거: NOT NULL UNIQUE와 PK의 효과는 같을지언정 의미가 다르다. 참조키는 기본키를 우선적으로 따른다.

-- 제약 조건 위반 시 테스트
-- 사용자명(NOT NULL)에 NULL 값을 입력 시도
INSERT INTO users (email, name)
VALUES ('ASDASD@naver.com', null); -- null이라 써도 되고 NULL이라 써도 됨.
-- Error Code: 1048. Column 'NAME' cannot be null

-- 이메일(UNIQUE)에 중복 값을 입력시도
INSERT INTO users (email, name)
VALUES ('ASDASD@naver.com', '홍');

INSERT INTO users (email, name)
VALUES ('ASDASD@naver.com', '길'); -- 위에서 들어가서 이메일 중복됨
-- Error Code: 1062. Duplicate entry 'ASDASD@naver.com' for key 'users.email'

-- 기본키(PK)에 중복 값을 입력 시도
INSERT INTO users (id, email, name)
VALUES (1, 'aasdd@naver.com', '동');
-- Error Code: 1062. Duplicate entry 'id1' for key 'users.PRIMARY'

-- 나이(age >= 18)에 18세 미만 값을 입력 시도
INSERT INTO users (email, name, age)
VALUES ('hihi@naver.com', '동', 15);
...
CREATE TABLE products (
  ...
  dc_rate INTEGER CHECK (dc_rate BETWEEN 0 AND 50),   		-- 할인율(0~50% 제한) , '0%' '50%'처럼 적는게 아닌가봄
...
-- 컬럼중에 값이 하나라도 수정되면 ON UPDATE가 발동됨
...
-- 외래키 제약 조건의 문법
-- CONSTRAINT [제약조건_이름]  -- 안넣으면 DB가 알아서 이름 지어줌
-- FOREIGN KEY ([자식_테이블의_컬럼명])  -- 필수로 적기
-- REFERENCES [부모_테이블명]([부모_테이블의_컬럼명])  -- 필수로 적기
-- [ON DELETE 옵션] [ON UPDATE 옵션] -- 옵션(적든 말든 상관X)
...
-- 특히 관계가 복잡하게 얽혀 있는 경우에는 파급 효과를 예측하기 어려움 (편리한데 위험함, 만약 참조하는게 여기저기 얽혀 있으면 삭제/업뎃하면 싹다 날라가니까) 
...
-- capitals 테이블 생성
CREATE TABLE capitals ( -- 자식
	id INT,
    name VARCHAR(255), -- 수도명
    country_id INT UNIQUE, -- 국가 아이디(수도가 속한 나라) 
    -- 타입은 국가 테이블과 동일하게 맞춤(부모의 id가 INT니까 INT로 적기)
...
-- 관례적으로 fk_참조하는쪽_참조당하는쪽 으로 이름 적어줌
-- 이름안적어주면 DB가 check_01 이런식으로 달아준다고함(이름이 뭘 표현하는건지 헷갈리겠지)
...
CREATE TABLE players (
    ...
    -- teams 테이블의 id 컬럼을 가리킴 => 선수가 어느 팀에 소속되어 있는지를 연결(관계 형성)
);
...
-- 다대다 관계에서는 중간 테이블을 만들어 외래키 지정 (중간테이블은 A,B의 외래키를 2개 갖고 있어야겠지.)
...
CREATE TABLE employees (
  ...
  FOREIGN (company_id) REFERENCES companies(id)  -- 외래키 지정: company_id
);
